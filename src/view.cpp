...
lv_obj_t * add_label(const char * text, lv_coord_t x_ofs, lv_coord_t y_ofs, lv_align_t align) {
  lv_obj_t * label = lv_label_create(lv_scr_act(), NULL);
  lv_label_set_text(label, text);
  lv_label_set_align(label, LV_LABEL_ALIGN_CENTER);
  lv_label_set_long_mode(label, LV_LABEL_LONG_EXPAND); 

  lv_obj_align(label, NULL, align, x_ofs, y_ofs);
...
lv_obj_t * add_image(const void * src_img, lv_coord_t x_ofs, lv_coord_t y_ofs, lv_event_cb_t event_handler)
{
    lv_obj_t * img1 = lv_img_create(lv_scr_act(), NULL);
    lv_img_set_src(img1, src_img);
    lv_obj_align(img1, NULL, LV_ALIGN_CENTER, x_ofs, y_ofs);
    lv_obj_set_click(img1, true);
    if(event_handler != NULL) {
      lv_obj_set_event_cb(img1, event_handler);
    }
    return img1;
}
